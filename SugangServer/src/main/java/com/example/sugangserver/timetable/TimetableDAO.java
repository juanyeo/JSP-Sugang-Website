package com.example.sugangserver.timetable;
...
public class TimetableDAO {
    ...
    public ArrayList<Lecture> getTimeTableList(String studentId, int currentSemester) {
        ...
        String SQL = "SELECT C.class_id, C.class_no, C.course_id, C.name, C.major_id, M.name, \n" +
                ...
                "WHERE 1=EXISTS(SELECT * FROM credits CR2 WHERE CR2.class_id = C.class_id AND CR2.student_id = " + studentId + ") " +
                "AND C.opened = " + String.valueOf(currentSemester) + ";";
    ...
    public int cancelRegister(String classId) {
        String SQL = "DELETE FROM credits WHERE student_id = ? AND class_id = ? AND grade IS NULL";
